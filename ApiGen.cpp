...
*/
    // unsupport printout
    fprintf(fp, "static void enc_unsupported(const char* fn)\n{\n\tPGA_TRACE(\"tid %%d: %%s: NOT SUPPORTED\\n\", gettid(), fn);\n}\n\n");
    ...
    for (size_t i = 0; i < size(); i++) 
    {
        ...
        fprintf(fp, "\tPGA_TRACE(\"tid %%d: %%s (");
        for (size_t j = 0; j < nvars; j++) {
            if (!e->vars()[j].isVoid()) {
                fprintf(fp, "%s%s",
                        j != 0 ? ", " : "",
                        e->vars()[j].type()->printFormat().c_str());
            }
        }
        fprintf(fp, ")\\n\", gettid(), __FUNCTION__");
        for (size_t j = 0; j < nvars; j++) {
        ...
        if (e->unsupported())
        {
            fprintf(fp, "\tenc_unsupported(__FUNCTION__);\n");
            if (shouldReturn)
        ...
        else
        {
            ...
            if (shouldReturn)
                fprintf(fp, "\t%s retval = ", e->retval().type()->name().c_str());
            else
                fprintf(fp, "\treturn ");

            fprintf(fp, "%s_enc (", e->name().c_str());
...
// gen xxx_dec dummy implementations.
// we return 0, typecast to return type, in case the function has a return type.
// This may be wrong for a particular case so be very careful about using these
// dummy functions.
//
// For any OES function, we generate a call to its non-OES counterpart
//
int ApiGen::genDummyImplementation(const std::string & filename, SideType side)
{

    if (side != SERVER_SIDE) {
        fprintf(stderr, "Dummy implementation is only defined for server components\n");
        return -999;
    }
    ...
    for (size_t i = 0; i < size(); i++) {
        ...
        e->print(fp, true, "_dec");
        fprintf(fp, "{\n");

        bool shouldReturn = !e->retval().isVoid();
        size_t nvars = e->vars().size();
#if 0
        fprintf(fp, "\tPGA_TRACE(\"%%s (");
        for (size_t j = 0; j < nvars; j++) {
            if (!e->vars()[j].isVoid()) {
                fprintf(fp, "%s%s",
                        j != 0 ? ", " : "",
        ...
        bool oes_fn = false;
        const char *c = e->name().c_str();
        char c1[512] = {0};
        // a quick and dirty check, should suffice
        if (strstr (c, "OES")) {
            oes_fn = true;
            strncpy (c1, c, strlen(c) - strlen ("OES"));
        }

        fprintf (fp, "\t%spga_%s (", shouldReturn ? "return " : "", oes_fn ? c1 : e->name().c_str());
...
int ApiGen::genOpcodes(const std::string &filename)
{
    ...
    size_t k;
    char spaces[80];
    for (k = 0; k < sizeof(spaces); k++)
        spaces[k] = ' ';
    spaces[k-1] = '\0';

    unsigned int j = (unsigned int)m_baseOpcode;

    k = fprintf(fp, "#define PGA_OP_%s_first", m_basename.c_str());
    fprintf(fp, "%s%u\n", &spaces[k], j);
    j++;

    for (size_t i = 0; i < size(); i++, j++) {
        k = fprintf(fp, "#define PGA_OP_%s", at(i).name().c_str());
        fprintf(fp, "%s%u\n", &spaces[k], j);
    }

    k = fprintf(fp, "#define PGA_OP_%s_last", m_basename.c_str());
    fprintf(fp, "%s%u\n", &spaces[k], j);
...
int ApiGen::genEncoderHeader(const std::string &filename)
{
    ...
    printHeader(fp);
    std::string classname = m_basename + "_encoder_h";

    fprintf(fp, "\n#ifndef __GUARD_%s\n", classname.c_str());
    fprintf(fp, "#define __GUARD_%s\n\n", classname.c_str());
...
//                classname.c_str(),
//                classname.c_str());
//        fprintf(fp, "\tPgaSocket *sock = l_ctx->m_sock;\n\n");

        /* add logging */
        size_t nvars = e->vars().size();
        fprintf(fp, "\tPGA_TRACE_ENC(\"tid %%d: %%s (");
        for (size_t j = 0; j < nvars; j++) {
            if (!e->vars()[j].isVoid()) {
                fprintf(fp, "%s%s",
                        j != 0 ? ", " : "",
        ...
        fprintf(fp, "\tPgaClientTls *tls = PgaClientTlsGet ();\n");
        fprintf(fp, "\tPgaSocket *sock = &(tls->s);\n\n");
        ...
        while (nvars < maxvars || maxvars == 0) {
            ...
            if (nvars == 0 || j > nvars) {
                ...
                if (nvars == 0 && j == maxvars) {
                    ...
                    fprintf(fp, "\tptr = (unsigned char*) PgaSocketAlloc(sock, packetSize);\n");

                } else {
                    // allocate buffer from the stream until the first large variable
                    //fprintf(fp, "\tptr = stream->alloc(");
                    fprintf(fp, "\tptr = (unsigned char*) PgaSocketAlloc(sock, ");
                    plus = "";
                ...
                if (nvars == 0) {
                    fprintf(fp, "\tint tmp = PGA_OP_%s; ", e->name().c_str());
                    fprintf(fp, "memcpy(ptr, &tmp, 4); ptr += 4;\n");
        ...
        fprintf(fp, "\tint tmp = PGA_OP_%s; memcpy(ptr, &tmp, 4); ptr += 4;\n",  e->name().c_str());
        fprintf(fp, "\tmemcpy(ptr, &packetSize, 4); ptr += 4;\n\n");
        ...
        for (size_t j = 0; j < nvars; j++) {
            if (evars[j].isPointer()) {
                ...
                if (dir == Var::POINTER_INOUT || dir == Var::POINTER_OUT) {
                    ...
                    fprintf(fp, "PgaSocketRecvCopy(sock, (char*)%s, __size_%s);\n",
                            varname, varname);
                }
            }
        }
        ...
        if (e->retval().isPointer()) {
            fprintf(fp, "\tPGA_TRACE_ENC(\"tid %%d: returning NULL from %%s (");
        } else if (e->retval().type()->name() != "void") {
            fprintf(fp, "\tPGA_TRACE_ENC(\"tid %%d: returning [%s] from %%s (", e->retval().type()->printFormat().c_str());
        } else {
            fprintf(fp, "\tPGA_TRACE_ENC(\"tid %%d: returning from %%s (");
        }
    ...
    fprintf(fp, "%s::%s(PgaSocket *sock)\n{\n", classname.c_str(), classname.c_str());
    fprintf(fp, "\tm_sock = sock;\n\n");
...
int ApiGen::genDecoderHeader(const std::string &filename)
{
    ...
    printHeader(fp);
    std::string classname = m_basename + "_decoder_h";
    ...
    for (size_t i = 0; i < size(); i++) {
        fprintf(fp, "\t");
        at(i).print(fp, false, "_dec", /* classname + "::" */"", /*"void *self"*/"");
        fprintf(fp, ";\n");
    }
    fprintf(fp, "};\n\n");
...
/* return true if this pointer is to be printed */
bool if_print_pointer (std::string name)
{
    /* deref only the following type of pointers to print values */
    ...
    std::string delim = "const ";       // XXXPKK : handle more than one space after const
...
/* prints debug logs in decoder */
void print_in_pointers (FILE *fp, EntryPoint *e)
{
            ...
            if (!v->isPointer()) {
                ...
                if (v->pointerDir() == Var::POINTER_IN || v->pointerDir() == Var::POINTER_INOUT) {
                    if (if_print_pointer(v->type()->name())) {
                        if (v->type()->name().find(std::string("GLchar")) != std::string::npos) {
                            fprintf (fp, "\t\tconst char* str = (const char*)(ptr + %s + 4); "
                                    "print_array (&str, sizeof(str));\n", varoffset.c_str());
                        } else {
                            fprintf (fp, "\t\tprint_array ((%s)(ptr + %s + 4), *(unsigned int *)(ptr + %s));\n",
                                    v->type()->name().c_str(), varoffset.c_str(), varoffset.c_str());
                        }
                    }
...
/* prints debug logs in decoder */
void print_out_pointers (FILE *fp, EntryPoint *e)
{
    for (size_t i = 0; i < e->vars().size(); i++) {
        Var *v = &e->vars()[i];
...
int ApiGen::genDecoderImpl(const std::string &filename)
{
    ...
    fprintf(fp, "\nGLint PgaDecode_%s (PgaSocket* sock, unsigned int opcode, unsigned int len, char* buf, bool dump)\n{\n", m_basename.c_str());
    fprintf(fp,
            "\tint handled = 0;\n\
\tif (len < 8) return 1; \n\
...
\tswitch(opcode)\n\t{\n");
    ...
    for (size_t f = 0; f < n; f++) {
		...
		fprintf(fp, "\t\tPGA_TRACE_DUMP(\"%s (%s)\\n\"", e->name().c_str(), printString.c_str());
            ...
            if (pass == PASS_DebugPrint) print_in_pointers (fp, e);
            ...
            if (pass == PASS_TmpBuffAlloc) {
                ...
                if (totalTmpBuffExist) {
                    ...
                    //fprintf(fp, "\t\tunsigned char *tmpBuf = (unsigned char*) malloc(totalTmpSize);\n");
                    fprintf(fp, "\t\tunsigned char *tmpBuf = (unsigned char*) PgaSocketAlloc (sock, (int)totalTmpSize);\n");
                }
            }
            ...
            if (pass == PASS_Epilog) {
                ...
                print_out_pointers (fp, e);

                // send back out pointers data as well as retval
                if (totalTmpBuffExist) {
                    //fprintf(fp, "\t\t\tstream->flush();\n");
                    //fprintf(fp, "\t\tPgaSocketAllocCopy (sock, (char *)tmpBuf, totalTmpSize);\n");
                }
            }

        } // pass;
        if (totalTmpBuffExist) {
            //fprintf(fp, "\t\tPGA_TRACE(\"%s : returned %%d\", *((int*)tmpBuf));\n", e->name().c_str());
            //fprintf(fp, "\t\tfree (tmpBuf);\n");
        }
        fprintf(fp, "\t\t}\n");
        fprintf(fp, "\t\tbreak;\n\n");
